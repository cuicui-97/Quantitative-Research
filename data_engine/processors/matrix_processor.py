...
class MatrixProcessor:
    ...
    def build_bse_matrix(
        ...
        Returns:
            DataFrame: 北交所股票矩阵（1=北交所 不可交易，0=非北交所 可交易）
        """
        logger.info("构建北交所股票矩阵...")

        if self.basic_info is None:
            logger.error("基础数据不存在")
            return pd.DataFrame()

        # 找出所有北交所股票
        bse_stocks = self.basic_info[self.basic_info['market'] == '北交所'].index.tolist()
        logger.info(f"识别到 {len(bse_stocks)} 只北交所股票")
        ...
        # 将北交所股票设为1（不可交易）- 向量化操作
        bse_stocks_in_matrix = matrix.columns.intersection(bse_stocks)
        if len(bse_stocks_in_matrix) > 0:
            matrix[bse_stocks_in_matrix] = 1

        logger.info(f"北交所股票不可交易比例: {matrix.mean().mean():.2%}")
        return matrix
    ...
    def build_tradability_matrix(
        ...
        综合判断：上市天数、北交所、ST、数据缺失、涨跌停
        任一条件不满足（值为1或非0）则不可交易

        注：停牌股票无交易数据，已被数据缺失矩阵捕获，无需单独判断
        ...
        # 2. 北交所股票矩阵
        logger.info("[2/6] 构建北交所股票矩阵...")
        bse_matrix = self.build_bse_matrix(dates, stocks)
        if save_intermediate:
            save_matrix(bse_matrix, Config.MATRIX_DATA_DIR / 'bse_matrix.csv')

        # 3. ST 状态矩阵
        logger.info("[3/6] 加载 ST 状态矩阵...")
        ...
        # 确保所有矩阵都是 int8 类型（避免位运算类型错误）
        listing_days_matrix = listing_days_matrix.astype(np.int8)
        bse_matrix = bse_matrix.astype(np.int8)
        st_matrix_aligned = st_matrix_aligned.astype(np.int8)
        missing_data_matrix = missing_data_matrix.astype(np.int8)

        # 合并所有条件（任一为1则不可交易）
        # 注：停牌股票无交易数据，已被数据缺失矩阵捕获，无需单独判断
        logger.info("[6/6] 合并所有条件...")
        tradability_matrix = (
            listing_days_matrix |
            bse_matrix |
            st_matrix_aligned |
