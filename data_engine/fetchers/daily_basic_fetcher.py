"""
日度基本面数据抓取器
使用 daily_basic API 获取股票日度基本面数据（市值、PE、PB 等）
"""
import pandas as pd
...
class DailyBasicFetcher(BaseFetcher):
    """日度基本面数据抓取器（使用 daily_basic API）"""
    ...
    def fetch_daily_basic(self, start_date: str, end_date: str, force_refresh: bool = False) -> pd.DataFrame:
        """
        使用 daily_basic API 获取日度基本面数据

        通过遍历交易日获取每日的全市场基本面数据
        智能识别缺失的日期范围并补充数据
        ...
        Returns:
            DataFrame: 包含 ts_code, trade_date, total_mv, circ_mv, pe, pb, ps 等字段
        """
        return self._fetch_by_date_range(start_date, end_date)

    @with_checkpoint('daily_basic_temp.csv')
    def _fetch_by_date_range(self, start_date: str, end_date: str) -> tuple:
        """
        遍历日期范围获取日度基本面数据
    ...
    def _fetch_single_date_with_retry(self, date_str: str) -> pd.DataFrame:
        ...
        Returns:
            DataFrame: 日度基本面记录
        """
        return self.api.fetch_daily_basic(trade_date=date_str)
