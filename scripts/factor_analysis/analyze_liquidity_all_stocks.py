...
def run_all_stocks_factor_analysis(factor_name: str, factor_matrix_file: str, enable_cost: bool, logger):
    """在全市场可交易股票中运行单个因子分析"""
    cost_tag = 'with_cost' if enable_cost else 'no_cost'

    logger.info(f"\n{'='*60}")
    logger.info(f"{factor_name} 全市场因子分析（{'含成本' if enable_cost else '不含'}）")
    logger.info(f"{'='*60}")
    ...
    logger.info(f"  可交易比例: {tradable_count / total_cells:.2%}")
    ...
    analyzer = SingleFactorAnalyzer(
        ...
        tradability_matrix=tradable,  # 直接使用可交易矩阵，不叠加微盘股筛选
        mv_matrix=mv,
        n_groups=10,
    ...
    output_dir = Config.DATA_DIR / 'factor_analysis_results' / 'all_stocks' / f'{factor_name.lower()}_{cost_tag}'
    ...
    plot_combined_returns(
        ...
        factor_name=f'{factor_name} (All Stocks)',
...
def main():
    logger = setup_logger(prefix="liquidity_all_stocks")

    logger.info("="*70)
    logger.info("全市场流动性因子分析")
    logger.info("="*70)
    ...
    for factor_name, matrix_file in factors:
        try:
            logger.info(f"\n\n{'#'*70}")
            logger.info(f"# 开始分析: {factor_name} (全市场)")
            logger.info(f"{'#'*70}")

            # 不含成本
            result_no_cost = run_all_stocks_factor_analysis(factor_name, matrix_file, False, logger)
            stats_no_cost = result_no_cost['stats_equal'].loc['Long-Short (10-1)']

            # 含成本
            result_with_cost = run_all_stocks_factor_analysis(factor_name, matrix_file, True, logger)
