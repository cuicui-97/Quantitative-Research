...
def run_factor_analysis(factor_name: str, matrix_file: str, enable_cost: bool, logger):
    """运行单个因子的分析"""
    cost_tag = 'with_cost' if enable_cost else 'no_cost'

    logger.info(f"\n{'='*60}")
    logger.info(f"{factor_name} 因子分析（{'含成本' if enable_cost else '不含'}）")
    logger.info(f"{'='*60}")

    # 加载矩阵
    yoy_matrix = load_matrix(Config.MATRIX_DATA_DIR / matrix_file)
    ...
    logger.info(f"  因子矩阵: {yoy_matrix.shape}")
    ...
    factor = yoy_matrix.loc[common_dates, common_stocks]
    ...
    output_dir = Config.DATA_DIR / 'factor_analysis_results' / f'{factor_name.lower()}_{cost_tag}'
...
def main():
    logger = setup_logger(prefix="factor_batch")
    ...
    factors = [
        ...
        ('TotalProfitYoY', 'total_profit_yoy_matrix.csv'),
        ('AdminExpYoY', 'admin_exp_yoy_matrix.csv'),
        ('FinExpYoY', 'fin_exp_yoy_matrix.csv'),
        ('SellExpYoY', 'sell_exp_yoy_matrix.csv'),
    ]
    ...
    for factor_name, matrix_file in factors:
        try:
            logger.info(f"\n\n{'#'*70}")
            logger.info(f"# 开始分析: {factor_name}")
            logger.info(f"{'#'*70}")

            # 不含成本
            result_no_cost = run_factor_analysis(factor_name, matrix_file, False, logger)
            stats_no_cost = result_no_cost['stats_equal'].loc['Long-Short (10-1)']

            # 含成本
            result_with_cost = run_factor_analysis(factor_name, matrix_file, True, logger)
