...
def run_microcap_factor_analysis(factor_name: str, factor_matrix_file: str, enable_cost: bool, logger):
    ...
    # 关键：合并可交易矩阵与微盘股矩阵
    # 微盘股内 且 可交易的股票：tradable == 0 且 microcap == 1
    # 新的可交易矩阵：非微盘股 或 不可交易 的设为1（不可交易）
    ...
    analyzer = SingleFactorAnalyzer(
        ...
        tradability_matrix=microcap_tradable,  # 使用微盘股筛选后的可交易矩阵
        mv_matrix=mv,
        n_groups=10,  # 使用10组进行分层
...
def main():
    logger = setup_logger(prefix="microcap_factor")

    logger.info("="*70)
    logger.info("微盘股内因子分析")
    logger.info("="*70)
