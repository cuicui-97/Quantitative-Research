...
def run_analysis(enable_cost: bool, logger):
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"归母净利润同比增速因子分析（{'含' if enable_cost else '不含'}交易成本）")
    logger.info(f"{'='*60}")

    # 加载矩阵
    yoy_matrix = load_matrix(Config.MATRIX_DATA_DIR / 'net_profit_yoy_matrix.csv')
    ...
    logger.info(f"  YoY矩阵: {yoy_matrix.shape}")
