...
def run_analysis(enable_cost: bool, logger):
    factor_name = 'RevenueYoY'
    cost_tag = 'with_cost' if enable_cost else 'no_cost'

    logger.info(f"\n{'='*60}")
    logger.info(f"营业总收入同比增速因子分析（{'含' if enable_cost else '不含'}交易成本）")
    logger.info(f"{'='*60}")

    # 加载矩阵
    yoy_matrix = load_matrix(Config.MATRIX_DATA_DIR / 'total_revenue_yoy_matrix.csv')
    ...
    logger.info(f"  营收YoY矩阵: {yoy_matrix.shape}")
    ...
    output_dir = Config.DATA_DIR / 'factor_analysis_results' / f'revenue_yoy_{cost_tag}'
