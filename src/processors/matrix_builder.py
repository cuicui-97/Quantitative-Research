...
class MatrixBuilder:
    ...
    def from_long_format(
        ...
        # 创建空矩阵
        matrix = pd.DataFrame(default_value, index=all_dates, columns=all_stocks)
        ...
        for _, row in tqdm(df.iterrows(), total=len(df), desc="处理记录"):
            ...
            if ts_code in matrix.columns and trade_date in matrix.index:
                if value_col and value_col in row:
                    matrix.loc[trade_date, ts_code] = row[value_col]
                else:
                    matrix.loc[trade_date, ts_code] = 1
