...
目前系统共有 **37个因子矩阵**，分为7大类：

| 类别 | 因子数量 | 主要因子 |
|------|---------|---------|
| 财务报表 YoY | 11 | 净利润YoY、营收YoY、EBITDA YoY等 |
| 量价技术 | 8 | 动量、反转、波动率、换手率等 |
...
| **Amihud非流动性** | `amihud_20d_matrix.csv` | 20日平均(\|收益\|/成交额)×1e9 | 流动性 proxy |
...
reversal_5d = -(close / close.shift(5) - 1)  # 反转取负
volatility_20d = returns.rolling(20).std()
amihud = (|returns| / amount).rolling(20).mean() * 1e9
```
...
| 因子 | 年化收益 | 夏普比率 | 特点 |
|------|---------|---------|------|
| **Amihud** | 22.39% | **1.06** | 全市场流动性因子 |
| **价量相关** | **164.14%** | **5.89** | 微盘股内表现极强（可能过拟合） |

**注意**: 价量相关系数(vp_corr_20d)在微盘股内表现异常强，需谨慎使用。
...
3. **概念因子**: 目前回测数据不完整，需进一步验证
4. **财务YoY**: 在微盘股内数据覆盖率低，效果不佳
...
| `scripts/matrix_building/build_price_volume_matrices.py` | 量价因子（5个） |
